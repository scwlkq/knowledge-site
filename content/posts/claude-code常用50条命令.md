...
> 我基于当前目录中的 `claude源码/src/commands.ts` 和 `claude源码/src/commands/*` 做了这份梳理。不同 Claude Code 版本、账号权限、实验开关、内部构建和插件安装状态会影响实际可见命令；我实际使用时会以 `/help` 显示为准。
...
- 上下文变长时，我优先用 `/compact`，不会一上来就 `/clear`。
- 开始一个风险较高的探索前，我会先 `/branch`，给自己留一条回来的路。
- 会话多起来以后，我会先 `/rename`，这样后面 `/resume` 时更容易找到。
...
- 提交前我至少会跑一次 `/diff`，确认 Claude 的改动范围符合预期。
- 遇到认证、IDE、MCP、模型或网络相关异常时，我会先看 `/status` 和 `/doctor`。
- 我通常把 `/context` 和 `/compact` 配合使用：先看占用，再决定是否压缩。
...
- 大改动我会先 `/plan`，确认方案后再让 Claude 修改代码。
- 简单任务我会降低 `/effort`；复杂设计、重构、排障时再提高。
- `/fast` 我更倾向于用在短平快任务上；长时间复杂任务我会留意成本和效果。
...
- 需要 Claude 读写当前项目外的目录时，我会用 `/add-dir`，而不是让它绕过权限。
- MCP 连接异常时，我会先用 `/mcp` 查看状态，再决定启用或禁用具体 server。
- 插件或 skill 刚安装后，如果当前会话没有生效，我会尝试 `/reload-plugins`。
...
- 提交前我会用 `/review` 找 bug、逻辑漏洞和遗漏测试。
- `/security-review` 我只把它当作安全审查，不把它等同于通用 code review。
- `/commit` 这类 prompt 命令会分析当前改动；在它之前，我仍然会先 `/diff`。
...
- IDE 无法联动时，我会先看 `/ide`，再看 `/doctor`。
- 远程控制相关命令通常受账号、组织策略和特性开关影响，我会先确认当前环境是否支持。
- `/install-github-app` 和 `/install-slack-app` 属于外部服务集成，执行前我会确认目标仓库或工作区权限。
...
如果当前 `/help` 看不到这些命令，我不会把它们写成内置命令；我会改成自然语言任务：
...
我把这份指南定位为“命令参考手册”：保留分类、参数和示例，但不强调个人经验排名，也不把未确认命令写成内置能力。

我实际使用时主要记住三点：

1. 我先用 `/help` 看当前版本真正可用的命令。
2. 对话变长时，我用 `/context` + `/compact` 管理上下文。
3. 改代码前我用 `/plan`，提交前我用 `/diff` + `/review`。
